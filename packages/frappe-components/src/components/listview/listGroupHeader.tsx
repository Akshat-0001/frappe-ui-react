...
const ListGroupHeader: React.FC<ListGroupHeaderProps> = ({ group: initialGroup }) => {
  const listContext = useContext(ListContext);
  const [collapsed, setCollapsed] = useState(initialGroup.collapsed ?? false);
  ...
  }, [listContext, initialGroup, collapsed]);
  ...
  return (
    <>
      <div className="flex items-center">
        ...
        >
          <DownSolid
            className={`h-4 w-4 text-ink-gray-6 transition-transform duration-200 ${collapsed ? '-rotate-90' : ''}`}
          />
        </button>
        {groupHeaderSlot}
      </div>
